<?php
class Privatepanel_SliderController extends Zend_Controller_Action
{
	...
	public function indexAction(){
 	...
 	public function addAction(){
		...
		$element = $form->getElement('slider_image_path');
				...
				if(is_object($media_path) and $media_path->success){
					...
					if(is_object($insertedMedia) and $insertedMedia->success){
						$mySession->successMsg = "Slider Image Successfully added "; // Sucessss
 						$this->_redirect("privatepanel/slider");
 					}
  				}
	...
	public function editAction(){
 	...
 	public function getmediaAction(){
		...
		foreach($qry as $row1)
		{
			...
			$row[]='<a class="btn default btn-xs purple" href="'.APPLICATION_URL.'/privatepanel/slider/edit/slider_image_id/'.$row1[$sIndexColumn].'"><i class="fa fa-edit"></i> Edit </a>';
		...
		$this->_helper->viewRenderer->setNoRender(true);
 		...
 		if ($this->getRequest()->isPost()) {
			...
			if(isset($formData['slider_images']) and count($formData['slider_images'])){
 			...
 			$this->_redirect('/privatepanel/slider/?removed=1');	 
