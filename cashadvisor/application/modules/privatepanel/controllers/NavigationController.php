			...
			if($form->isValid($data_form)){
				...
				if(!$approval_required){
					...
					if($is_insert->success){
						...
						$this->_redirect('privatepanel/navigation');
					}
 	...
 	public function editAction(){
		...
		$form->populate($content);
 		...
 		if($this->getRequest()->isPost()) { 
			...
			if(!$block_edit){
				$mySession->successMsg = " Menu Block Values are Same as previous ";
				$this->_redirect("privatepanel/navigation");
			}
		...
		foreach($qry as $row1)
		{
			...
			$row[]=date('d-F , Y',strtotime($row1['menu_updated']));
  			...
  			if($row1['menu_status']=="1"){
   			...
   			$row[]='<a class="btn mini green-stripe" href="'.APPLICATION_URL.'/privatepanel/navigation/edit/menu_id/'.$row1[$sIndexColumn].'">Edit</a>';
			$row[]='<a class="btn mini green-stripe" href="'.APPLICATION_URL.'/privatepanel/navigation/view/menu_id/'.$row1[$sIndexColumn].'">View</a>';
 			$output['aaData'][] = $row;
			$j++;
		}	
