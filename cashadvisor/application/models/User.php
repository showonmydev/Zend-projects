...
class Application_Model_User extends Zend_Db_Table_Abstract
{
	 ...
	 */
 	public function add($data , $id = false){	
		...
		$password=$data['user_o_password'];
		unset($data['user_o_password']);
		try{
			...
			if(isset($data['user_login_type']) && $data['user_login_type']=='social')
			{
				$isSend = $this->modelEmail->sendEmail('social_registration_email',$data);
			}
			else
			{
	 ...
	 */
	 ...
	 public function getallUserList(){
		$this->_name = "users";
		$result = $this->select()
					   ->where("user_status='1' and user_type='recruiter'")
		               ->Query()
					   ->fetchAll();
	 ...
	 function getRecruiterList(){
		...
		$c=$this->getallUserList();
		
		$OptionsArr = array();
		/*$OptionsArr[0]['key'] = "";
		$OptionsArr[0]['value'] = "Select Recruiters";
		*/
		$k = 1;
		foreach($c as $values)
		{
			$OptionsArr[$k]['key'] = $values['user_id'];
			$OptionsArr[$k]['value'] = $values['user_first_name']." ".$values['user_last_name'] ;
			$k++;
		}
